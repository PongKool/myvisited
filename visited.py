...
def get_thailand_time_str():
  return datetime.now(ZoneInfo("Asia/Bangkok")).strftime(
      "%Y-%m-%d %I:%M:%S %p"
  )
...
def geocode_place(place_name):
  ...
  try:
    ...
    location = geolocator.geocode(
        place_name.strip(), timeout=5, addressdetails=True
    )
    if location:
      ...
      return (
          location.latitude,
          location.longitude,
          clean_province_name(raw_province),
      )
  except Exception:
    pass
  return None, None, "Unknown"
...
def save_entry(
  ...
  existing_match = df[
      df["Place Name"].str.strip().str.lower() == clean_name.lower()
  ]
...
# Page Config
st.set_page_config(
    page_title="Visited Places Log", page_icon="📍", layout="wide"
)

# Display pending toast messages after a rerun
if "toast_msg" in st.session_state:
  st.toast(
      st.session_state.toast_msg["text"], icon=st.session_state.toast_msg["icon"]
  )
  del st.session_state.toast_msg

st.title("📍 Visited Places Log & Map")
st.write(
    "Record and visualize all the amazing places you have visited on an"
    " interactive map."
)
...
with col_left:
  ...
  if loc and "coords" in loc:
    ...
    autofilled_name, autofilled_province = reverse_geocode(
  ...
  gps_mode = st.radio(
      ...
      horizontal=True,
      help=(
          "Manual: Click on map or select province to locate. Use current GPS:"
          " Auto-detect location. Look with GPS: Search coordinates for typed"
          " place name."
      ),
  )
  ...
  if use_gps and autofilled_name:
    ...
    st.info(
        f"📍 Location Picked on Map: ({clicked_lat:.4f}, {clicked_lon:.4f})"
    )
  ...
  rating = st.slider(
      "Rating Score (0 - 10)", min_value=0, max_value=10, value=5, step=1
  )
  ...
  if use_gps:
    ...
    st.caption(
        "🗓️ **Visit Date & Time:** Auto-set to current time"
        f" ({now_bkk.strftime('%Y-%m-%d %I:%M:%S %p')})"
    )
  else:
  ...
  selected_datetime_str = datetime.combine(visit_date, visit_time).strftime(
      "%Y-%m-%d %I:%M:%S %p"
  )
  ...
  with col_num:
    num_people = st.number_input(
        "Number of People", min_value=1, value=1, step=1
    )
  with col_comp:
    companions = st.text_input(
        "Who went with you?",
        placeholder="e.g., Alice, Bob (Leave empty for Solo)",
    )

  notes = st.text_area(
      "Notes / Memories",
      placeholder="e.g., Had great seafood, sunset was amazing!",
  )
  ...
  if st.button("📌 Record Place", type="primary"):
    if not place_name.strip():
      ...
      if lat and lon:
        ...
        st.session_state.toast_msg = {
            "text": (
                f"⚠️ Logged '{place_name}', but couldn't locate it on the map."
            ),
            "icon": "⚠️",
        }
        st.rerun()
...
with col_right:
  ...
  map_output = st_folium(
      ...
      height=400,
      key=(
          f"visited_map_{gps_mode}_{province}_{prov_lat}_{prov_lon}_{clicked_lat}_{clicked_lon}_{manual_lat}_{current_lat}"
      ),
  )
...
if not data.empty:
  ...
  m_col4.metric(
      "Mapped Locations",
      (
          unique_map_places["Place Name"].nunique()
          if not unique_map_places.empty
          else 0
      ),
  )
  ...
  with col_sort_dir:
    sort_ascending = (
        st.radio("Direction:", ["Ascending", "Descending"], horizontal=True)
        == "Ascending"
    )
  ...
  column_config = {
      "No.": st.column_config.NumberColumn(
          "No.", min_value=1, step=1, disabled=True
      ),
      "Rating": st.column_config.NumberColumn(
          "Rating", min_value=0, max_value=10, step=1, format="%d"
      ),
      "Notes": st.column_config.TextColumn("Notes"),
      "Category": st.column_config.SelectboxColumn(
          "Category", options=CATEGORIES, required=True
      ),
      "Visit Count": st.column_config.NumberColumn(
          "Visit Count", min_value=1, step=1
      ),
      "Number of People": st.column_config.NumberColumn(
          "Number of People", min_value=1, step=1
      ),
...
view_mode = st.radio(
    "Inspect visits by:",
    ["By Location", "By Province", "By Rating Score"],
    horizontal=True,
)
...
if view_mode == "By Location":
  ...
  selected_place = st.selectbox(
      "Select a place to inspect visits:", unique_places
  )
  if selected_place:
    place_visits = edited_df[
        edited_df["Place Name"] == selected_place
    ].sort_values(by="Last Visited", ascending=False)
    if len(place_visits) > 1:
      visit_options = [
          f"Visit #{row['Visit Count']} - {row['Last Visited']} (Rating:"
          f" {row['Rating']}/10, {row['Number of People']} people:"
          f" {row['Companions']})"
          for _, row in place_visits.iterrows()
      ]
      selected_visit_label = st.selectbox(
          "Select specific visit record:", visit_options
      )
    ...
    st.info(
        ...
        f" {chosen_visit['Last Visited']}\n\n👥 **Group Size:**"
        f" {chosen_visit['Number of People']} person(s)"
        f" (**Companions:** {chosen_visit['Companions']})\n\n📝 **Notes:**"
        f" {chosen_visit['Notes'] if pd.notna(chosen_visit['Notes']) and chosen_visit['Notes'] else 'No notes added'}"
    )

elif view_mode == "By Province":
  unique_provinces = sorted(edited_df["Province"].dropna().unique())
  selected_province = st.selectbox(
      "Select a province to inspect visits:", unique_provinces
  )
  if selected_province:
    prov_visits = edited_df[
        edited_df["Province"] == selected_province
    ].sort_values(by="Last Visited", ascending=False)
    st.success(
        f"Found **{len(prov_visits)}** visit record(s) in"
        f" **{selected_province}** across"
        f" **{prov_visits['Place Name'].nunique()}** unique place(s)."
    )
    visit_options = [
        f"{row['Place Name']} - Visit #{row['Visit Count']} on"
        f" {row['Last Visited']} (Rating: {row['Rating']}/10,"
        f" {row['Number of People']} people: {row['Companions']})"
        for _, row in prov_visits.iterrows()
    ]
    selected_visit_label = st.selectbox(
        "Select specific visit record in this province:", visit_options
    )
    selected_index = visit_options.index(selected_visit_label)
    ...
    st.info(
        f"📍 **{chosen_visit['Place Name']}** ({chosen_visit['Province']})\n\n🏷️"
        f" **Category:** {chosen_visit['Category']}\n\n⭐ **Rating:**"
        f" {chosen_visit['Rating']} / 10\n\n🗓️ **Date:**"
        f" {chosen_visit['Last Visited']}\n\n👥 **Group Size:**"
        f" {chosen_visit['Number of People']} person(s)"
        f" (**Companions:** {chosen_visit['Companions']})\n\n📝 **Notes:**"
        f" {chosen_visit['Notes'] if pd.notna(chosen_visit['Notes']) and chosen_visit['Notes'] else 'No notes added'}"
  ...
  selected_rating = st.selectbox(
      "Select a rating score to inspect visits:",
      unique_ratings,
      format_func=lambda r: f"⭐ {r} / 10",
  )
  if selected_rating is not None:
    score_visits = edited_df[edited_df["Rating"] == selected_rating].sort_values(
        by="Last Visited", ascending=False
    )
    st.success(
        f"Found **{len(score_visits)}** visit record(s) rated"
        f" **{selected_rating}/10** across"
        f" **{score_visits['Place Name'].nunique()}** place(s)."
    )
    visit_options = [
        f"{row['Place Name']} ({row['Province']}) - Visit #{row['Visit Count']}"
        f" on {row['Last Visited']} ({row['Number of People']} people:"
        f" {row['Companions']})"
        for _, row in score_visits.iterrows()
    ]
    selected_visit_label = st.selectbox(
        "Select specific visit record with this score:", visit_options
    )
    selected_index = visit_options.index(selected_visit_label)
